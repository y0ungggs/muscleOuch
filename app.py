...
with tab2:
    ...
    st.subheader("1️⃣ 일자별 인증 횟수 Top 10")
...
with tab3:
    ...
    st.subheader("2️⃣ 전체 동호회원 누적 인증")
    ...
    st.subheader("3️⃣ 동호회원별 누적 인증 (선택)")
...
with tab4:
    ...
    st.subheader("2️⃣ Z-score 기준 상위 동호회원")
...
with tab5:
    ...
    # 0. 많은 참여 감사 인사
    st.info("안녕하세요. 근육통 총무 김영수입니다🙇‍♂️  \n 팀전으로 처음 진행한 제 1회 운동 인증회에 참여해주신 모든 동호회원분들께 진심으로 감사드립니다!")

    # 1. 3회 이상 인증한 사람의 수 및 안내
    certified_3plus = df.groupby("이름")["인증"].sum()
    certified_3plus_count = (certified_3plus >= 3).sum()
    st.subheader("🎊3회 이상 인증한 회원 수🎊")
    st.info(f"총 37명의 근육통 회원 중 {certified_3plus_count}명의 회원이 3회 이상 인증을 완료하셨습니다.  \n"
           "해당 인원분들께는 곧 치킨 기프티콘을 선물드릴 예정입니다!  \n그동안 열심히 운동하셨으니 즐기세요🍗"
           )
    ...
    st.subheader("🏆가장 많이 인증한 팀 및 팀원 안내🏆")
    st.info(f"가장 많이 인증한 팀은 🎉🎉**{top_team_name}**입니다🎉🎉  \n 총 인증 횟수는 **{top_team_cert_sum}회** 입니다.  \n축하드립니다")

    # 해당 팀 소속 팀원 및 인증 횟수
    top_team_members = df[df["팀"] == top_team_name].groupby("이름")["인증"].sum().reset_index().sort_values("인증", ascending=False)
    st.dataframe(top_team_members.rename(columns={"이름": "팀원 이름", "인증": "총 인증 횟수"}))

    st.info(
        "위 팀원분들께는 15만원 상당의 신발 구매를 지원해드립니다.😄  \n"
        "  ※ 이전 블랙리스트에 포함된 **박채은**회원님은 5만원 상당의 운동용품 구매를 지원해드립니다.🤭  \n"
        "미리미리 골라두세요~"
    )

    # 3. 50회 이상 인증 회원 및 사다리타기 안내
    certified_50plus = certified_3plus[certified_3plus >= 50].reset_index().rename(columns={"이름": "회원명", "인증": "인증 횟수"})
    st.subheader("👑50회 이상 인증한 회원 명단👑")
    if len(certified_50plus) > 0:
        st.dataframe(certified_50plus)
        st.info(
            "50회 이상 인증한 회원 중 사다리타기로 1명을 선정하여 15만원 상당의 신발 구매를 지원해드립니다.😄  \n"
            "🙏이번 우승 팀원과 기존 블랙리스트 회원을 제외합니다."
        )
        st.info(
            "이에 금번 사다리타기 대상자는 총 7명입니다.(가나다순)  \n"
            "🤗***고한솔, 박운학, 윤균성, 장명기, 장서혁, 장선우, 장창수***  \n"   
            "근육통 단체 카톡방에서 공정하게 사다리타기를 진행하겠습니다!!"
        )
    else:
